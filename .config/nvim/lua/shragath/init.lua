...
vim.api.nvim_create_autocmd("FileType", { pattern = "rust", callback = function()
    vim.keymap.set('i', '<F9>', '<cmd>TermExec cmd="cargo run" size=60<cr>')
    vim.keymap.set('n', '<F9>', '<cmd>TermExec cmd="cargo run" size=60<cr>')
    vim.keymap.set('n', '<F10>', '<cmd>TermExec cmd="cargo run --bin expand("%:t:r")"  size=60<cr>')
...
vim.api.nvim_create_autocmd("FileType", { pattern = "python", callback = function()
    ...
    vim.keymap.set('v', '<leader>r', '<cmd>MagmaEvaluateVisual<cr>')
