...
local R_group = vim.api.nvim_create_augroup("RAU", { clear = true })
...
vim.api.nvim_create_autocmd("FileType", {
    pattern = "rust",
    callback = function()
        vim.keymap.set({ 'n', 'i' }, '<F10>', '<esc><cmd>TermExec cmd="cargo test" size=60<cr>')
        vim.keymap.set({ 'n', 'i' }, '<F9>', '<esc><cmd>TermExec cmd="cargo run" size=60<cr>')
        vim.keymap.set({ 'n', 'i' }, '<F11>', '<esc><cmd>TermExec cmd="cargo run --bin %:t:r"  size=60<cr>')
...
vim.api.nvim_create_autocmd("FileType", {
    pattern = "r",
    callback = function()
        vim.keymap.set({ 'i', 'n' }, '<F9>', '<esc><cmd>TermExec cmd="Rscript %" size=60<cr>')
...
vim.api.nvim_create_autocmd("FileType", {
    pattern = "cs",
    callback = function()
        vim.keymap.set('i', '<F9>', '<esc><cmd>TermExec cmd="dotnet run" size=60<cr>')
        vim.keymap.set('n', '<F9>', '<esc><cmd>TermExec cmd="dotnet run" size=60<cr>')
    end,
    group = csharp_group
})
...
vim.api.nvim_create_autocmd("FileType", {
    pattern = "python",
    callback = function()
        vim.keymap.set('i', '<F9>', '<esc><cmd>TermExec cmd="python3 %" size=60<cr>')
        vim.keymap.set('n', '<F9>', '<esc><cmd>TermExec cmd="python3 %" size=60<cr>')
