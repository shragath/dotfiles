...
if (vim.api.nvim_exec('echo argc()', true) == "0") then
    ...
    local footer = {
        ...
        val = {total_plugins .. " plugins  " .. datetime},
        opts = {
            position = "center",
            hl = "Constant"
        }
    ...
    local buttons = {
        type = "group",
        val = {
            button("e", "  New Buffer", ':tabnew<CR>'), button("f", "  Find file", ':Telescope find_files<CR>'),
            button("h", "  Recently opened files", ':Telescope oldfiles<CR>'), button("r", "  Frecency/MRU", ':Telescope oldfiles<CR>'),
            button("g", "  Search sessions", ':SearchSession<CR>'), button("m", "  Word Finder", ':Telescope live_grep<CR>'),
